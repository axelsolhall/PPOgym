...
class PPONetwork(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.shared_layers = self.build_layers(
            self.input_dim, self.hidden_dims, normalize=True
        )

        # Policy head layers
        self.policy_layers = self.build_layers(
            self.hidden_dims[-1], self.policy_hidden_dims, normalize=True
        )
        self.policy_output = nn.Linear(self.policy_hidden_dims[-1], self.output_dim)

        # Value head layers
        self.value_layers = self.build_layers(
            self.hidden_dims[-1], self.value_hidden_dims, normalize=True
        )
...
class PPOWrapper:
        ...
        for _ in range(self.batch_epochs):
            ...
            for i in range(0, len(states), self.batch_size):
                ...
                value_loss = (
                    self.value_coef
                    * 0.5
                    * (batch_returns - new_value.squeeze()).pow(2).mean()
                )

                # Entropy loss
                entropy = -torch.sum(
                    new_policy_probs * torch.log(new_policy_probs + 1e-8), dim=-1
                ).mean()
    ...
    def adjust_learning_rate(self, current_epoch, total_epochs):
        ...
        new_lr = (self.initial_lr - self.final_lr) * (
            1 - current_epoch / total_epochs
        ) + self.final_lr
    ...
    def anneling(self, current_epoch, total_epochs):
        ...
        if self.anneal_entropy:
            if self.initial_entropy is None:
                ...
                self.entropy_coef = self.initial_entropy * (
                    1 - current_epoch / total_epochs
                )
    ...
    def evaluate_policy(self, episodes):
        ...
        while not np.all(dones):
            ...
            actions = np.zeros(
                episodes, dtype=int
            )  # Default action for inactive envs (e.g., action 0)

            if np.any(active_envs):  # Ensure there are active environments
                policies, _ = self.predict(
                    torch.tensor(states[active_envs], dtype=torch.float32)
                )
                actions[active_envs] = torch.argmax(
                    policies, dim=1
                ).numpy()  # Only predict for active envs

            # Step the environments with actions for all envs
            next_states, rewards, done_flags, truncated_flags, infos = envs.step(
                actions
            )
        ...
        for i in range(episodes):
            if dones[i]:
                print(
                    f"\t - Env {i}: reward = {scores[i]:.2f}, steps taken = {steps_taken[i]}"
                )
    ...
    def train_model(self, episodes, display=False):
        ...
        for i in range(episodes):
            ...
            # Run an episode
            states, actions, rewards, log_probs, dones, values = self.run_episode(
                self.episode_steps
            )
            ...
            if display:
                print(
                    f"Episode: {i}\t avg life: {self.avg_lifetime:.2f}, eval reward: {eval_reward:.2f}, eval ma: {self.eval_ma:.2f}"
                )
    ...
    def checkpoint(self, eval_reward, episode, total_epochs):
        ...
        try:
            # Restore logic
            if (
                ratio < self.ma_load_ratio
                and episode_spacing > self.minimum_episode_spacing
            ):
                self.load_model(os.path.join(self.checkpoint_path, self.model_save))
                self.load_optimizer(
                    os.path.join(self.checkpoint_path, self.optimizer_save)
                )
                with open(os.path.join(self.checkpoint_path, self.ma_save), "r") as f:
                    self.eval_ma = float(f.read())

                # Print in red
                output = "\033[91mRestored model from episode {}\033[00m".format(
                    self.latest_checkpoint_episode
                )
                print(output)
                self.latest_checkpoint_episode = episode
...
def sweep_worker(
    parameter_name, value, episodes, env_kwargs, network_kwargs, ppo_kwargs
):
...
def parameter_sweeper(
    ...
    for value in parameter_values:
        ...
        sweeps[value] = {"mean": result_mean.tolist(), "std": result_std.tolist()}
...
def plot_sweep_results(filename, moving_average=None):
    ...
    for i, (param, values) in enumerate(data.items()):
        ...
        if moving_average:
            ...
            ax.fill_between(
                x, mean_ma - std_ma, mean_ma + std_ma, color=colors[i], alpha=0.07
            )
        else:
